...
def rnn_backward(dh, cache):
    ...
    for t in range(T-1, -1, -1):
      ...
      dnext_h = dh[:, t, :] + dprev_h # 上边+右边， 初始右边为0
...
def word_embedding_forward(x, W):
    ...
    ##############################################################################
    out = W[x] # (N,T, D) x中的每个元素，都会取出一个D维数组
    cache = (W, x)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def word_embedding_backward(dout, cache):
    ...
    (W, x) = cache
    dW = np.zeros(W.shape)
    np.add.at(dW, x, dout) # 按照x中的index将dout中的梯度加到dw中，出现多次就加多次
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    return dW
...
def lstm_backward(dh, cache):
    ...
    for t in range(T-1, -1, -1):
      dnext_h = dh[:, t, :] + dprev_h # 上面+右面， 初始右边=0
