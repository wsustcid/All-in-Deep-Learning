...
def batchnorm_backward(dout, cache):
    ...
    Inputs:
    - dout: Upstream derivatives, of shape (N, D) (same to the input)
    ...
    dx3= dxhat / a
    dvar= np.sum(dxhat * (x-mean) * -0.5 * a**(-3), axis =0) # (D,)
    dmean2 = np.sum(-dxhat / a, axis =0)
    
    dmean1 = dvar * np.sum(-2 * (x-mean), axis =0) / N # =0
    dx2 = dvar * 2 * (x - mean) / N
    dmean = dmean1 + dmean2
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for i in range(H_out):
        for j in range(W_out):
            ...
            for k in range(F):
                out[:, k , i, j] = np.sum(x_padded_mask * w[k, :, :, :], axis=(1,2,3)) # C H W -> 1 for all samples
    
    out += b[None, :, None, None] # (N,F,H_out,W_out)
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(H_out):
        for j in range(W_out):
            x_padded_mask = x[:, :, i*stride:i*stride+pool_height, j*stride:j*stride+pool_width]
            max_mask = np.max(x_padded_mask, axis=(2,3)) # (N,C)
