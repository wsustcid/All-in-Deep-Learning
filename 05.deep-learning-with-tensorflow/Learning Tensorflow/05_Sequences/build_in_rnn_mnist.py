...
@LastEditTime: 2020-05-15 22:41:12
'''
...
log_dir = os.path.join(base_dir, "logs/build_in_rnn")
...
def get_model(inputs):    
    with tf.name_scope("build_in_rnn") as scope:
        
        rnn_cell = tf.contrib.rnn.BasicRNNCell(hidden_layer_size) # like rnn_step()
        outputs, _ = tf.nn.dynamic_rnn(rnn_cell, inputs, dtype=tf.float32) # like tf.scan() get all hidden states
        ...
        last_rnn_output = outputs[:,-1,:] # B, T, H
        # just get final time output
        output = get_linear_layer(last_rnn_output)
