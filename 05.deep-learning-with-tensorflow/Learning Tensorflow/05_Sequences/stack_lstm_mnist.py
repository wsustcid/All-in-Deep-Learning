...
@LastEditTime: 2020-05-16 17:10:43
'''
...
Error:
cells=[lstm_cell]*num_LSTM_layers : 报错维度不一致；可能是由于这样两个cell其实地址相同；
"""
import os
...
def get_model(inputs):    
    with tf.name_scope("multi-lstm") as scope:

        lstm_cell = tf.nn.rnn_cell.BasicLSTMCell(hidden_layer_size, forget_bias=1.0)
        cell = tf.nn.rnn_cell.MultiRNNCell(cells=[copy.deepcopy(lstm_cell) for i in range(num_lstm_layers)], state_is_tuple=True)
        outputs, states = tf.nn.dynamic_rnn(cell, inputs, time_major=False, dtype=tf.float32)  
        ...
        final_output = get_linear_layer(states[num_lstm_layers-1][1]) # 等价于 outputs[:,-1,:]
